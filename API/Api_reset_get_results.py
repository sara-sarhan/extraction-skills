...
path=  os.getcwd()[0:os.getcwd().rfind("\\")]

@app.route('/services/start', methods=['POST'])
def start():
   import json
   import json
   with open( os.path.join(path, "json",'json_data_info.json')) as json_file:
      data = eval(json.load(json_file))
   json_string = json.dumps(data)
    # Directly from dictionary
   data['Stato']='run'
   json_string = json.dumps(data)
   with open( os.path.join(path, "json",'json_data_info.json'), 'w') as outfile:
