    ...
    if n and directory is not None:
        ...
        with pd.ExcelWriter(path, engine='xlsxwriter') as writer1:
         ...
         for   pdfnamepath in  files :
            ...
            with open(os.path.join(os.getcwd(), "json",'json_data_info.json'), 'w') as outfile:
                 json.dump(json_string, outfile)
            category, skillsner,address_,mails_,name_,data_ = predict.main(pdfnamepath)
            names_sur.append(name_)
            adress.append(address_)
            mails.append(mails_)
            birthday.append(data_)
            ...
            card = dbc.Card(
                [
                dbc.CardHeader(name_+ ' tipo lavoro '+category.capitalize() ,
         ...
         df = pd.DataFrame({"nomiFiles": files,'nome e cognome':names_sur,'residenza o domicilio': adress,'mails':mails,'data nascita':birthday}) #, "category": category
         df.to_excel(writer1, sheet_name='info curriculum', index=False)
